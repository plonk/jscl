...
;;   (let ((function (coerce-to-function function)))
...
(defun sequence-start-end (fun seq start end)
  (values start (or end (length seq)) (length seq)))

(defun fill-array-with-elt (arr item start end)
  (do ((i start (1+ i)))
      ((= i end) arr)
...
(defun copy-subarray (vec1 start1 vec2 start2 length)
  (setq length
        (min (- (length vec1) start1) (- (length vec2) start2) length))
  (if (and (eq vec1 vec2)
           (< start2 (+ start1 length)))
      (loop
         :for i from (1- (+ start1 length)) downto start1
         :for j from (1- (+ start2 length)) downto start2
         do
           (setf (aref vec1 i) (aref vec2 j)))
      (loop
         :for i from start1 below (+ start1 length)
         :for j from start2 below (+ start2 length)
         do
           (setf (aref vec1 i) (aref vec2 j)))))
    ...
    (with-start-end (start end in l)
      (with-count (%count count :output in)
        (let* ((existing 0))
          ...
          (unless (eq out in)
            (setf existing 
                  (apply #'count `(,which ,in :start ,start :end ,end
                           ,@(if test (list :test test))
                           ,@(if test-not (list :test-not test-not))
                           :key ,key)))
            (when (zerop existing)
          ...
          (let ((skip 0))
            ...
            (when from-end
              (unless (plusp existing)
                (setf existing
                      ;; (count which in :start start :end end
...
;;   (remove (coerce-to-function predicate) sequence
...
(defun delete-if (predicate sequence &key (start 0) end from-end count key)
  (delete (coerce-to-function predicate) sequence
          :start start :end end :from-end from-end :count count
          :test #'unsafe-funcall1 :key key))
...
(defun substitute-if (new predicate sequence
  ...
  (nsubstitute new (coerce-to-function predicate) (copy-seq sequence)
...
(defun nsubstitute-if (new predicate sequence
                       &key (start 0) end from-end count key)
  (nsubstitute new (coerce-to-function predicate) sequence
               :key key :test #'unsafe-funcall1
               :start start :end end :from-end from-end :count count
               :key key))
...
(defun find-if (predicate sequence &key from-end (start 0) end key)
  (find (coerce-to-function predicate) sequence
        :from-end from-end :start start :end end
        :test #'unsafe-funcall1 :key key))
...
;;   (position (coerce-to-function predicate) sequence
;;             :from-end from-end :start start :end end
;;             :test #'unsafe-funcall1 :key key))
...
;;   (setf key (if key (coerce-to-function key) #'identity)
;;         predicate (coerce-to-function predicate))
;;   (if (listp sequence)
...
(defun make-sequence (result-type size)
  (ecase result-type
    (list
     (let ((r nil))
       (dotimes (n size)
